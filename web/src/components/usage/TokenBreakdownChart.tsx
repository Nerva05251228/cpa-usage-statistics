...
const TOKEN_COLORS: Record<TokenCategory, { border: string; bg: string }> = {
  ...
  cacheWrite: { border: '#06b6d4', bg: 'rgba(6, 182, 212, 0.25)' },
  unclassified: { border: '#ef4444', bg: 'rgba(239, 68, 68, 0.20)' },
  reasoning: { border: '#8b5cf6', bg: 'rgba(139, 92, 246, 0.25)' }
};

const CATEGORIES: TokenCategory[] = ['input', 'output', 'cached', 'cacheWrite', 'reasoning', 'unclassified'];
...
export function TokenBreakdownChart({
  ...
  const { chartData, chartOptions } = useMemo(() => {
    ...
    const categoryLabels: Record<TokenCategory, string> = {
      input: t('usage_stats.uncached_input_tokens'),
      output: t('usage_stats.non_reasoning_output_tokens'),
      cached: t('usage_stats.cached_tokens'),
      reasoning: t('usage_stats.reasoning_tokens'),
      cacheWrite: t('usage_stats.cache_write_tokens'),
      unclassified: t('usage_stats.unclassified_tokens')
    };
