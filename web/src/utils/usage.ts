...
import { getTokenAccounting, priceTokenAccounting } from './usage/tokenAccounting';
import type { CanonicalTokenBreakdown, CostEstimate, TokenPrice } from './usage/tokenAccounting';

export { getTokenAccounting } from './usage/tokenAccounting';
export type { CanonicalTokenBreakdown, CostEstimate, TokenAccounting } from './usage/tokenAccounting';
...
export interface UsageDetail {
  ...
  auth_index: number;
  latency_ms?: number;
  token_breakdown?: CanonicalTokenBreakdown;
...
const getZonedDayFormatter = (timeZone: string): Intl.DateTimeFormat => {
  const normalized = timeZone.trim() || DEFAULT_USAGE_TIME_ZONE;
  const cached = zonedDayFormatterCache.get(normalized);
  ...
  try {
    ...
    formatter.format(0);
    zonedDayFormatterCache.set(normalized, formatter);
    return formatter;
  } catch {
    const fallback = getZonedDayFormatter(DEFAULT_USAGE_TIME_ZONE);
    zonedDayFormatterCache.set(normalized, fallback);
    return fallback;
  }
};

const getZonedDayKey = (timestamp: number, timeZone: string): string => {
  const parts = getZonedDayFormatter(timeZone).formatToParts(timestamp);
...
export function getStartOfDayInTimeZone(
  ...
  const targetDay = getZonedDayKey(resolvedNowMs, timeZone);
  let low = resolvedNowMs - 48 * 60 * 60 * 1000;
  let high = resolvedNowMs;

  while (low < high) {
    const middle = Math.floor((low + high) / 2);
    if (getZonedDayKey(middle, timeZone) < targetDay) {
      low = middle + 1;
    } else {
      high = middle;
    }
...
export function filterUsageByTimeRange<T>(
  ...
  const windowStart =
    range === 'today'
      ? getStartOfDayInTimeZone(nowMs, timeZone)
    ...
    Object.entries(models).forEach(([modelName, modelEntry]) => {
      ...
      modelDetails.forEach((detailRaw) => {
        ...
        details.push({
          ...
          client_source:
            typeof detailRaw.client_source === 'string'
              ? detailRaw.client_source.trim()
              : typeof detailRaw.clientSource === 'string'
                ? detailRaw.clientSource.trim()
                : undefined,
          auth_index: detailRaw.auth_index as unknown as number,
          latency_ms: latencyMs ?? undefined,
          tokens: tokensRaw as unknown as UsageDetail['tokens'],
          token_breakdown: detailRaw.token_breakdown as UsageDetail['token_breakdown'],
          legacy_accounting: detailRaw.legacy_accounting === true,
...
export function extractTotalTokens(detail: unknown): number {
  return getTokenAccounting(detail).totalTokens;
}
...
export function calculateTokenBreakdown(usageData: unknown): TokenBreakdown {
  ...
  details.forEach((detail) => {
    const accounting = getTokenAccounting(detail);
...
export function getCostEstimate(
  ...
  return priceTokenAccounting(getTokenAccounting(detail), modelPrices[detail.__modelName || '']);
}
...
export function calculateCost(
  ...
  return getCostEstimate(detail, modelPrices).amount;
}
...
export function getApiStats(
  usageData: unknown,
  modelPrices: Record<string, ModelPrice>,
  resolveLabel: (id: string) => string = maskUsageSensitiveValue,
): ApiStats[] {
  const apis = getApisRecord(usageData);
  if (!apis) return [];
  ...
  Object.entries(apis).forEach(([endpoint, apiData]) => {
    ...
    result.push({
      id: endpoint,
      endpoint: resolveLabel(endpoint) || endpoint,
...
export type TokenCategory = 'input' | 'output' | 'cached' | 'cacheWrite' | 'reasoning' | 'unclassified';
...
export function buildHourlyTokenBreakdown(
  ...
  const dataByCategory: Record<TokenCategory, number[]> = {
    ...
    unclassified: new Array(labels.length).fill(0),
  };
  ...
  details.forEach((detail) => {
    ...
    const accounting = getTokenAccounting(detail);
    dataByCategory.input[bucketIndex] += accounting.inputTokens;
    dataByCategory.output[bucketIndex] += accounting.outputTokens;
    dataByCategory.cached[bucketIndex] += accounting.cachedTokens;
    dataByCategory.cacheWrite[bucketIndex] += accounting.cacheWriteTokens;
    dataByCategory.reasoning[bucketIndex] += accounting.reasoningTokens;
    dataByCategory.unclassified[bucketIndex] += accounting.unclassifiedTokens;
    hasData = true;
  });
...
export function buildDailyTokenBreakdown(usageData: unknown): TokenBreakdownSeries {
  ...
  details.forEach((detail) => {
    ...
    if (!dayMap[dayLabel]) {
      dayMap[dayLabel] = { input: 0, output: 0, cached: 0, cacheWrite: 0, reasoning: 0, unclassified: 0 };
    }

    const accounting = getTokenAccounting(detail);
    dayMap[dayLabel].input += accounting.inputTokens;
    dayMap[dayLabel].output += accounting.outputTokens;
    dayMap[dayLabel].cached += accounting.cachedTokens;
    dayMap[dayLabel].cacheWrite += accounting.cacheWriteTokens;
    dayMap[dayLabel].reasoning += accounting.reasoningTokens;
    dayMap[dayLabel].unclassified += accounting.unclassifiedTokens;
    hasData = true;
  });
  ...
  const dataByCategory: Record<TokenCategory, number[]> = {
    ...
    unclassified: labels.map((l) => dayMap[l].unclassified),
  };
